...
public class GameRequest {
    ...
    private void setup() {
        ...
        requestMessage.getClient().on(ButtonInteractionEvent.class)
                .takeWhile(e -> GameManager.gameRequestExists(this))
                .filter(e -> e.getMessageId().equals(requestMessage.getId())
                        && players[1].equals(e.getInteraction().getMember().get()))
    ...
    public void create(InteractionContext context) {
        ...
        requestMessage = context.getChannel().getMessageById(Snowflake.of(
                context.event.getInteractionResponse().getInitialResponse().block().id())).block();
        setup();
    }

    private void onOpponentButton(ButtonInteractionEvent event) {
